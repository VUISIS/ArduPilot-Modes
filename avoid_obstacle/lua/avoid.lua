...
function update()
  -- gcs:send_text(0, "LUA: Mission Item Test")
  local current_mode = vehicle:get_mode()
  gcs:send_text(0, string.format("The current flight mode is : %d", current_mode))
  if current_mode == 0 then
    if vehicle:set_mode(19) then
      gcs:send_text(0, "The flight mode has been changed to 19 (ADSB Avoidance Mode)")
    else
      gcs:send_text(0, "The flight mode change has failed")
    end
  elseif current_mode == 19 then
    if vehicle:set_mode(0) then
      gcs:send_text(0, "The flight mode has been changed back to 0 (Stabilize Mode)")
    else
      gcs:send_text(0, "The flight mode change has failed")
    end
  end
...
function update_mission() -- this is the loop which periodically runs
